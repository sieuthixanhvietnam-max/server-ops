...
/** Icon-badge stat card. Fixed height + a single-line value (ellipsized
 * rather than wrapped) so a short label like "Tổng domain" and a longer one
 * like "Đồng bộ domain" always render the same box size in the same row -
 * antd's Row already stretches Col height evenly, but Card only fills that
 * space when told to (height: 100%), otherwise it just hugs its own content
 * and the tiles end up visibly uneven despite the equal-height grid under them. */
const KpiCard: React.FC<{
  ...
  // an exact-timestamp hover (see "Đồng bộ domain"/"Tổng CF Account" below).
